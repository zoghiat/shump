...
public class GameManager
{
    ...
    private void HandleShooting(float deltaTime)
    {
        ...
        if (!isShooting || shootTimer < fireDelay || !Player.CanShoot())
        {
            return;
        }
    ...
    private void DrawHud(Graphics g)
    {
        ...
        if (Player.IsTripleShotActive)
        {
            g.DrawString("Triple Shot", font, Brushes.Orange, 10, y);
        }

        if (Player.IsShieldActive)
        {
            g.DrawString("Shield", font, Brushes.DeepSkyBlue, 120, y);
        }

        if (Player.IsFireRateBoostActive)
        {
            g.DrawString("Fire Booster", font, Brushes.Violet, 210, y);
        }
    }
}
